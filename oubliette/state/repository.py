...
class Repository(Protocol):
    ...
    def set_attuned(self, char_id: str, item_ids: list[str]) -> None: ...
...
class InMemoryRepository:
    ...
    def set_attuned(self, char_id: str, item_ids: list[str]) -> None:
        """Absolute attuned-bond list (item ids, max 3). Validation (cap, carried,
        requires-attunement) happens on the live path before the op is produced
        (D6); replay trusts the recorded list."""
        self.get_character(char_id).attuned = list(item_ids)
