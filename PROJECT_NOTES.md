...
**D-CTRL-1 — control-spell signatures (full repo suite green; new `test_ctrl_spells.py` +25, new `ctrl_lab`). All five sub-mechanics DONE; NOT yet live-verified.** Order was OublietteDev's call: CTRL first, then the geometry pair.
...
**D-AOE-1 — AoE shape geometry (full repo suite green; new `test_aoe_shapes.py` +14; new `aoe_lab`). DONE, NOT yet live-verified.** Cone/line/cube no longer resolve as radius bursts.
