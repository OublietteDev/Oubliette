...
**🐛 KNOWN BUG (deferred to next session, OublietteDev's report):** after **casting a spell**, **Practiced Shove (a bonus action) becomes unusable** that turn — OublietteDev "made it work" with some ordering workaround. Practiced Shove is NOT a spell (no `spell_level`), so the RAW "one bonus-action spell + only-a-cantrip-action" rule shouldn't apply. Suspects to check: (a) does some cast path wrongly mark the **bonus-action** slot used (or set a "cast a leveled spell this turn → no other spell" guard that catches non-spell bonus actions)?; (b) GUI radial-menu availability not refreshing the bonus action after an action resolves; (c) a concentration interaction. Likely a `can_use_action_type`/`_mark_action_type_used` or GUI-gating issue, NOT the shove action itself (it works fine cast first). Repro: aoe_lab — cast Spike Growth, then try Practiced Shove.
...
**REMAINING to close Phase D:** just **D-WALL-1** (walls as real barriers; `wall_damage_on_enter` is dead code; wall spells currently route through a circular zone instead of a blocking line). The line geometry from D-AOE-1 (`hex_line` + `aoe_shapes`) is the shared groundwork it builds on. See `docs/roadmap/oubliette-phase-d-plan-v0.1.md`.
