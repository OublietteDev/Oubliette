...
**REMAINING in D-SPELL/COND/ACT (the audit's final package):** D-ZONE-1 (zone friendly-fire), D-ACT-3 (cover vs DEX saves), D-ACT-4 (ranged long-range + in-melee disadvantage), then the heavier D-WALL-1, D-CTRL-1, D-ACT-1 (Ready), D-ACT-2 (player Grapple), D-AOE-1 (AoE shape geometry). See `docs/roadmap/oubliette-phase-d-plan-v0.1.md`.
