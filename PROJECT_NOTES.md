...
- **Bench:** `wall_lab` + `wall_warden` ("Mason the Wallwright", L11, all six walls, full slots 1-6) + `wall_lab.bat`. **GOTCHA reaffirmed:** the GUI two-click flow + preview aren't exercised by any unit test (pygame); the manager path they call IS fully tested — so this needs OublietteDev's live playtest (cast a barrier across the Raiders' lane → they detour; drop Wall of Fire across the lane → they cook crossing it).
