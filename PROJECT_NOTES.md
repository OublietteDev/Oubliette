...
- **`ctrl_lab`** + new lab caster **`characters/control_warden.json`** ("Cassia the Confounder" — level-11 wizard chassis off Elara with full slots 1-6 and all five control spells appended; built so one hero can demo everything). Pack of Ogre+4 goblins clustered for Chain Lightning; a Bruiser starts adjacent to Cassia so a Confusion "lash out" has a neighbour.

**REMAINING in D-SPELL/COND/ACT (closes the audit + Phase D):** the two heaviest — **D-AOE-1** (AoE cone/line/cube geometry; everything currently resolves as a radius burst) and **D-WALL-1** (walls as real barriers; `wall_damage_on_enter` is dead code). They share line/shape geometry — build it once for both. (Also queued under D-CTRL-1's tail per OublietteDev's RAW note: Spike Growth forced-movement per-5ft, once a hex-line helper exists.) See `docs/roadmap/oubliette-phase-d-plan-v0.1.md`.
