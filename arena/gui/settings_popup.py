"""In-fight options popup: two live sliders the tavern playtest asked for.
...
Both persist through ``save_settings`` on release, so the choice sticks across
fights and sessions.
"""
...
class SettingsPopup(Popup):
    ...
    def __init__(self, screen_width: int, screen_height: int) -> None:
        ...
        self.rect.height = (
            self.TITLE_HEIGHT + 2 * _ROW_H + self.BTN_HEIGHT + 2 * self.PADDING
        )
